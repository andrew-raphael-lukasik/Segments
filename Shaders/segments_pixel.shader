...
Properties
{
    ...
    [Header(Width)]
    _WidthPx ( "Width (Pixels)" , Float ) = 4.0
...
SubShader
{
    ...
    Pass
    {
        ...
        float distancepx(float3 worldPosA, float3 worldPosB)
        {
            // world -> clip
            float4 clipA = TransformWorldToHClip(worldPosA);
            float4 clipB = TransformWorldToHClip(worldPosB);
            ...
            // NDC -> piksele
            float2 pxA = (ndcA * 0.5 + 0.5) * _ScreenParams.xy;
            float2 pxB = (ndcB * 0.5 + 0.5) * _ScreenParams.xy;

            return length(pxB - pxA);
        }

        float widthpxtoworld(float pixels, float3 worldPos, float3 worldDir)
        {
            float pxPerWorldUnit = distancepx(worldPos, worldPos + worldDir);
            return pixels / pxPerWorldUnit;
        }
        ...
        void geom ( line Varyings IN[2] , inout TriangleStream<geomOut> STREAM )
        {
            ...
            float bWidth = widthpxtoworld(_WidthPx, bottom.vertexW.xyz, widthDir);
            float tWidth = widthpxtoworld(_WidthPx, top.vertexW.xyz, widthDir);
            ...
            float bDist = distancepx(blWS, brWS);
            float tDist = distancepx(tlWS, trWS);
